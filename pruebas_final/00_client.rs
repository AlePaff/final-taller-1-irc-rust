...
fn client_run(address: &str, stream: &mut dyn Read) -> std::io::Result<()> {
    ...
    for line in reader.lines() {
        ...
        if let Ok(line) = line {
            ...
            socket.write(line.as_bytes())?;     //le mando el mensaje al servidor
...
//abro dos terminales
// para el servidor
// cargo r --bin 00-server 8080
// lo que esta a la derecha del -- es para cargo y lo de la izquierda es para el programa

// para el cliente
// cargo r --bin 00-client -- 127.0.0.1 8080

//luego escribo en el cliente y se lo manda al servidor
//lo cierro con Ctrl + C

//esto funciona identico en otra computadora, basta poner la dirección de otra computadora en el cliente y listo
